...
def main():
    ...
    ts = timestamp()
    logging.info("Starting Occupation Coding...")
    ...
    logging.info("Occupation Successfully Coded.")
...
I ran the main.py file and it works but the main function has several missing parts and the accuracy is about 0.76. The reason is because I don't see incorporated in the main.py the original file that I created for the ML model and I shared in the chat in the past. That file is attached here I please incorporate it correctly as it is especially the main loop where shuffling, gridsearch and stratisfied shuffling are incorporated. It is the missing of these parts that gives an accuracy of 0.76. If these parts are included the accuracy is higher above 0.8 
...
def build_param_grid():
    ...
    },
    {
        ...
        "classifier__C": [0.5, 1.0, 2.0, 5.0, 10.0],
        "classifier__penalty": ["l1", "l2"],  # l1 only with liblinear
        "classifier__class_weight": [None, "balanced"],
    }
]
...
def main():
    ...
    # --- Normalize and filter target columns to 4-digit ISCO codes ---
    df1[TARGET_COL] = df1[TARGET_COL].map(normalize_code_str)
    df2[TARGET_COL] = df2[TARGET_COL].map(normalize_code_str)

    # Keep only rows where target is exactly 4 digits
    df1 = df1[df1[TARGET_COL].str.fullmatch(r"\d{4}")]
    df2 = df2[df2[TARGET_COL].str.fullmatch(r"\d{4}") | df2[TARGET_COL].isna()]
